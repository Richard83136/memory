...
for ($i = 0; $i < $_SESSION['pairNmbr']*2; $i++) {
    $f = $i + 1;
    $cards[$i] = new Card($i, './img/img'.$i.'.jpg', './img/BackCard.jpg');
    $cards[$f] = new Card($f, './img/img'.$i.'.jpg', './img/BackCard.jpg');
    $i++;     
}
    ...
    if (strlen($key) == 5) {
        ...
        if ($charToVerify % 2 != 0) {
            ...
            if (
                ...
                $_SESSION['tata' . $charToVerify . $lastChar] = 'yes';
            }
            ...
            if (
                ...
                $_SESSION['tata' . $lastChar . $charToVerify] = 'yes';
            }
        }
    } elseif (strlen($key) == 6) {
...
function isEnd() {
    ...
    if ($tataNumber == $_SESSION['pairNmbr']) {
        $_SESSION['end'] = 'yes';
...
function adduser($username, $final) {    
    ...
    $query = "INSERT INTO score_utilisateurs (id, name, score) VALUES (null, '$username', '$final')";
    $mysqli->query($query);    
}

function showTopScorers() {
    $mysqli = new mysqli('localhost', 'root', '', 'memory');
    $query = "SELECT name, score FROM score_utilisateurs ORDER BY score DESC LIMIT 10";
    ...
    <?php if ($_SESSION['end'] == 'yes'):
        ...
        <div class="end-game__container">
            ...
            <div class="end-ann_container">
                <h1>Gagné !!!</h1>
                <?php if ($finalCount != 0): ?>
                <h3>Votre score est : <?=$finalCount?></h3>
                <div class="end-ann_container__add-block">
                    <form action="" method="post">
                        <h4>Entrez votre nom ci-dessous: </h4>
                        <input type="text" name="username">
                        <input type="submit" class="restart" name="adduser" value="Ajouter un vainqueur">
                    </form>
                </div>                
                <?php endif;?>                 
